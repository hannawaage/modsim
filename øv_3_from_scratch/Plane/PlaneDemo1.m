...
% M = eye(3);
% for i = 1:3
%     MakeArrow( O, M(:,i),0.8*ScaleFrame,FS,1.1*SW,'','facealpha', 0.5, 'color', Col{i})
%     CircArrow( O, O + 0.4*ScaleFrame*M(:,i),1,0.2,1,'facealpha', 0.5, 'color', Col{i}); 
% end
% text(0.*ScaleFrame,2.*ScaleFrame,-1.*ScaleFrame,{['$$\phi=$$',num2str(180*phi/pi, '%+.2f'),'$$^\circ$$'],['$$\theta=$$',num2str(180*theta/pi, '%+.2f'),'$$^\circ$$'],['$$\psi=$$',num2str(180*psi/pi, '%+.2f'),'$$^\circ$$']},'verticalalignment','bottom','horizontalalignment','center','fontsize',FS,'interpreter','latex')
% text(0.*ScaleFrame,0.*ScaleFrame,+2.2*ScaleFrame,'$$R_b^a = R_3(\psi)R_2(\theta)R_1(\phi)$$','verticalalignment','bottom','horizontalalignment','left','fontsize',FS,'interpreter','latex')
% 
% ViewAngleTable = linspace(0,2*pi,360);
% for k = 1:1%length(ViewAngleTable)
%     ViewAngle = ViewAngleTable(k);
%     Rview = [cos(ViewAngle)  -sin(ViewAngle)     0;
%              sin(ViewAngle)   cos(ViewAngle)     0;
%                  0           0       1];
...
for c = 1:4
    ...
    for k = 1:cN(c)+1
        ...
        [Rac,M] = Rotations1([phi;theta;psi]);
